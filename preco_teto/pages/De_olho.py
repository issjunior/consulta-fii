...
if not dados_btc.empty:
    ...
    ultimo_preco = float(dados_btc['Close'].iloc[-1])
    primeiro_preco = float(dados_btc['Close'].iloc[0])
    preco_maximo = float(dados_btc['High'].max())
    preco_minimo = float(dados_btc['Low'].min())
    
    with col1:
        st.metric("Preço Atual", f"{dolar(ultimo_preco)}")
    with col2:
        variacao = ((ultimo_preco - primeiro_preco) / primeiro_preco) * 100
        st.metric("Variação no Período", f"{porcentagem(variacao)}")
    with col3:
        st.metric("Máxima no Período", f"{dolar(preco_maximo)}")
    with col4:
        st.metric("Mínima no Período", f"{dolar(preco_minimo)}")
...
with col1:
    if not dados_btc.empty:
        ...
        st.dataframe(dados_btc, use_container_width=True)
    else:
