...
def obter_cdi():
    try:
        # Obtém os dados do CDI acumulados nos últimos 5 anos
        cdi_5anos = sgs.get(4389)  # Código '4389' para o CDI no SGS do Banco Central
        ...
        # Retorna 4 objetos default em caso de erro
        return pd.DataFrame(), pd.DataFrame(), pd.to_datetime("today").normalize() - pd.DateOffset(years=5), pd.to_datetime("today").normalize()
...
def criar_grafico_cdi(cdi_filtrado_formatado, data_inicio_12meses, data_corte):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode='markers+lines',
            name="CDI",
            hovertemplate="<b>Data:</b> %{x}<br><b>CDI:</b> %{y}<extra></extra>",
        )
    )
