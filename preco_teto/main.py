...
def main():
    ...
    try:
        ...
        print(f"O preço teto seria de {VERMELHO}R$ {preco_teto:.2f}{RESET} com o spread de {AMARELO}{spread:.2f}%{RESET}")
        print(f"O magic number do {AMARELO}{ticker.replace('.SA', '')}{RESET} é de {AMARELO}{cotas_necessarias}{RESET} cotas")
        print(f"Seria necessário {AMARELO}R$ {valor_cotas_magicnumber:.2f}{RESET} para alcançar a quantidades de cotas do magic number")
